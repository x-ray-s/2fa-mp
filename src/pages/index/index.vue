...
<template>
  <div class="h-full bg-gray-100 py-6 sm:py-12">
    <div class="relative h-full py-3 sm:mx-auto sm:max-w-xl">
        ...
        <div class="mx-auto h-full max-w-md ">
          <div class="flex h-full flex-col">
            <div class="space-y-4 p-4 text-base leading-6 text-gray-700 sm:text-lg sm:leading-7">
              ...
              <EditBar v-show="isEdit" :name="authCodes?.[editIndex]?.name" @esc="isEdit = false"
                @edit="isEditName = true" @delete="isDeleteOTP = true"
                @move="(direction) => moveItem(editIndex, direction)" />
            </div>

            <div class="scroll-none grow overflow-y-auto overflow-x-hidden text-base leading-6 sm:text-lg sm:leading-7">
              <ul>
                <li v-for="(code, index) in authCodes" :id="`item-${index}`" :key="index"
                  class="relative overflow-hidden p-4 pb-0" :class="{ 'active ': activeItem === index }"
                  @click="(event) => handleItemClick(index, event)" @longtap="() => handleItemLongTap(index)"
                  @touchend="handleItemLongTapEnd">
                  ...
                  <div class="flex items-center justify-between">
                    ...
                    <div class="relative size-6 shrink-0 rounded-full bg-blue-600">
                      <div
                        class="progress-circle absolute left-1/2 top-1/2 size-7 -translate-x-1/2 -translate-y-1/2 rounded-full"
                  ...
                  <div v-show="isEdit && editIndex === index" class="item-active" />
      ...
      <!-- 浮动按钮 -->
      <button
        class="fixed bottom-8 right-8 flex size-12 items-center justify-center rounded-full bg-gray-100 p-4  text-3xl text-blue-600 "
      ...
      <div v-if="showOptions" class="fixed inset-0 flex items-end justify-center bg-black/50">
        <div class="w-full rounded-t-3xl bg-white p-4 shadow-lg">
          <div class="mx-auto max-w-md">
            ...
            <button
              class="mt-4 block w-full rounded-xl bg-gray-100 px-4 py-2 text-sm text-gray-700 transition-colors duration-300 hover:bg-gray-200"
    ...
    <AddModal :is-open="showAddModal" @close="showAddModal = false" @save="(data:AuthCode) => {
        addAuthCode([data])
      }" />
